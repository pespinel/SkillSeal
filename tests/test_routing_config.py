...
from skillseal.routing.runner import RoutingConfigError, load_routing_config


def test_load_valid_config(tmp_path: Path) -> None:
    (tmp_path / "skillseal.yaml").write_text(
        "routing:\n  should_trigger:\n    - hello\n  should_not_trigger:\n    - goodbye\n"
    )
...
def test_empty_case_lists(tmp_path: Path) -> None:
    (tmp_path / "skillseal.yaml").write_text("routing:\n  should_trigger: []\n")
    config = load_routing_config(tmp_path)
    assert config is not None
...
def test_non_mapping_routing_raises(tmp_path: Path) -> None:
    (tmp_path / "skillseal.yaml").write_text("routing: not-a-mapping\n")
    with pytest.raises(RoutingConfigError):
        load_routing_config(tmp_path)
